...
def only_3d_heatmap_collate_fn(batch):
...
def create_only_3d_heatmap_data_loaders(datasets):
    ...
    train_loader = DataLoader(train_dataset, batch_size=config_file['batch_size'], shuffle=True, collate_fn=only_3d_heatmap_collate_fn) # Adjust batch size as needed
    test_loader = DataLoader(test_dataset, batch_size=config_file['batch_size'], shuffle=True, collate_fn=only_3d_heatmap_collate_fn)
...
class VideoClassifierHeatmapLSTM(nn.Module):
    def __init__(self, hidden_dim, num_layers, output_dim, fc_dropout, lstm_dropout, bidirectional_lstm):
        ...
        self.resnet3d = r3d_18(pretrained=True)  # Use pre-trained 3D ResNet-18
        self.resnet3d.stem[0] = nn.Conv3d(1, 64, kernel_size=(3, 7, 7), stride=(1, 2, 2), padding=(1, 3, 3), bias=False)
        input_dim = self.resnet3d.fc.in_features
        ...
        self.resnet3d.fc = nn.Identity()  # Remove final FC layer of ResNet
    ...
    def forward(self, heatmap, lengths):
        ...
        # packed_data = pack_padded_sequence(cnn_features, lengths, batch_first=True, enforce_sorted=True)
        output = self.dropout(cnn_features)
        output = self.fc(output)  # Take hidden state of the last LSTM layer
        return output
       
def create_only_3d_heatmap_model(output_dim):
...
def test_model_only_3d_heatmap(test_loader, model, criterion):
    correct = 0
    top_5_correct = 0
    total = 0
    running_loss = 0.0
...
def train_loop_only_3d_heatmap():
    train_loader, test_loader = create_only_3d_heatmap_data_loaders(datasets = config_file['datasets'])
    ...
    model = create_only_3d_heatmap_model(num_classes)
    ...
    for epoch in range(1, num_epoch+1):
        ...
        avg_test_accuracy, avg_top5_test_accuracy, avg_test_loss, test_prediction_results = test_model_only_3d_heatmap(test_loader, model, criterion)
    ...
    save_model_result(model, current_time, num_classes, avg_accuracy_list, avg_test_accuracy_list, avg_top5_test_accuracy_list, avg_loss_list, avg_test_loss_list, test_prediction_results)
